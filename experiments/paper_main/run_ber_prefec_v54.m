% Auto-split from NCKH_v53.m (original line 7956).
% Folder: experiments/paper_main

function pkg = run_ber_prefec_v54(cfg, vars, base, mc)
% PAM4 pre-FEC BER with identical channel as the paper's Fig T2-6,
% but with SNR sweep extended to reveal the 1e-3 ... 1e-5 waterfall.
 
    % ---- configuration (identical to paper's SER experiment) ------------
    cfg_p = cfg;                             % inherit PAM4 (M=4, A=[-3 -1 1 3])
    cfg_p.chan_mode = 'baseline_2tap';
    cfg_p.h_isi     = [1 0.5];               % same as Fig T2-6
    cfg_p.Nsym      = 80000;                 % enough for 1e-5 BER
    if isfield(cfg_p,'std8023'), cfg_p.std8023.enable = false; end
 
    fprintf('[ber_prefec_v54] PAM4 (M=%d), channel h=[%s]\n', ...
            cfg_p.M, num2str(cfg_p.h_isi, '%.2f '));
    fprintf('[ber_prefec_v54] Nf=%d, Nb=%d, D=%d, trainLen=%d\n', ...
            cfg_p.Nf, cfg_p.Nb, cfg_p.D, cfg_p.trainLen);
 
    % ---- Proposed variant: use vars.theorem (soft, well-tested) ---------
    v_prop = vars.theorem;
    base_p = base;
 
    % ---- SNR sweep -------------------------------------------------------
    snr_list = 8:2:26;                       % target BER 1e-1 ... <1e-5
    Nsnr = numel(snr_list);
    Nalg = 6;
    BER = zeros(Nsnr, Nalg);
    SER = zeros(Nsnr, Nalg);
    ...
    fprintf('[ber_prefec_v54] Nt=%d trials, %d SNR points\n', Nt, Nsnr);
 
    N_postTrain = cfg_p.Nsym - cfg_p.trainLen;
    total_bits = N_postTrain * Nt * log2(cfg_p.M);
    bit_floor = 1 / total_bits;
 
    fprintf('[ber_prefec_v54] %.2e bits per SNR point  (BER floor %.1e)\n', ...
    ...
    for t = 1:Nt
        ...
        for si = 1:Nsnr
            ...
            % *** NO AGC *** (matches paper's experiments)
        ...
        if mod(t,4)==0, fprintf('  [ber_prefec_v54] trial %d/%d\n', t, Nt); end
    end
    SER = SER / Nt;
    % PAM4 Gray coding:  BER ≈ SER / log2(M) in high-SNR regime
    BER = SER / log2(cfg_p.M);
 
    names = {'Proposed (theorem)','LMS','NLMS','RLS', ...
             'SM-sign-NLMS VSS','SM-sign-NLMS'};
 
    % ---- Print SER table ------------------------------------------------
    fprintf('\n[ber_prefec_v54] PAM4 SER  (%.2e symbols per SNR pt):\n', N_postTrain*Nt);
    fprintf('  SNR ');
    ...
    % ---- Print BER table ------------------------------------------------
    fprintf('\n[ber_prefec_v54] PAM4 pre-FEC BER  (%.2e bits per SNR pt):\n', total_bits);
    fprintf('  SNR ');
    for a = 1:Nalg, fprintf('%18s ', names{a}); end
    fprintf('\n');
    for si = 1:Nsnr
    ...
    % ---- SNR thresholds -------------------------------------------------
    fprintf('\n[ber_prefec_v54] SNR thresholds (dB) to reach pre-FEC BER target:\n');
    ...
    % ---- plot BER -------------------------------------------------------
    figure('Name','BER-Fig (v54): PAM4 pre-FEC BER');
    ...
    title(sprintf('PAM4 on h=[%s]: pre-FEC BER vs SNR', num2str(cfg_p.h_isi,'%.2f ')));
    ...
    grid on; xlabel('SNR (dB)'); ylabel('pre-FEC BER');
    title('Proposed vs SM-sign-NLMS VSS [2024]');
    legend({'Proposed (theorem variant)', ...
            'SM-sign-NLMS VSS (sign-innovation floor)'}, ...
           'Location','southwest');
    ylim([bit_floor/3, 1]);
    ...
    figure('Name', sprintf('EYE-Fig (v54): PAM4 pre-FEC eye @ SNR=%d dB', snr_target));
    clf; tiledlayout(1,2,'TileSpacing','compact','Padding','compact');
    nexttile; eye_plot_reshape_fixed(y_b_os, cfg_p.sps_eye);
    title(sprintf('Eye BEFORE (PAM4 ISI+AWGN, SNR=%d dB)', snr_target)); grid on;
