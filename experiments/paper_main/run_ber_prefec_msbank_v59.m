% Auto-split from NCKH_v53.m (original line 8693).
% Folder: experiments/paper_main

function pkg = run_ber_prefec_msbank_v59(cfg, vars, base, mc)
    cfg_p = cfg;
    ...
    state_est_acc = 0;          % accuracy of online state estimator
 
    fprintf('[ber_prefec_msbank_v59] MSBank-EQ, S=%d banks, Nf=%d Nb=%d\n', ...
            numel(cfg_p.markov.h2_states), cfg_p.Nf, cfg_p.Nb);
    fprintf('[ber_prefec_msbank_v59] Nt=%d, %d SNR points\n', Nt, Nsnr);
    ...
    for t = 1:Nt
        ...
        if mod(t,4)==0, fprintf('  [ber_prefec_msbank_v59] trial %d/%d\n', t, Nt); end
    end
    BER = BER / Nt;
    state_est_acc = state_est_acc / Nt;
 
    fprintf('\n[ber_prefec_msbank_v59] BER table:\n');
    for si = 1:Nsnr
        if err_count(si) < 0.5
    ...
    fprintf('\n[ber_prefec_msbank_v59] State estimator accuracy (post-training): %.2f%%\n', ...
            100*state_est_acc);
 
    fprintf('\n[ber_prefec_msbank_v59] SNR thresholds:\n');
    ...
    figure('Name','BER-Fig (v59): MSBank-EQ contribution'); clf;
    semilogy(snr_list, BER_disp, '-o', ...
             'Color', [0 0.45 0.74], 'LineWidth', 2.4, ...
             'MarkerFaceColor', [0 0.45 0.74], 'MarkerSize', 9);
    ...
    title(sprintf('Markov-State-Bank Equalizer (S=%d, state-est acc %.1f%%)', ...
...
% ============================================================================
% SECTION-D  —  PATCH C : SIDE-BY-SIDE COMPARISON (v59)
% ============================================================================
